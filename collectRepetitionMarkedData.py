...
def writeToFile(exercise, filenum, data, rep_markers):
    # Write to file for repetition recognition
    f = open("rep-data/{0}/{0}-{1}.txt".format(exercise,filenum), "w")
    f.write(str(data)+"\n")  
    f.write(str(rep_markers))
    f.close()
    # Write to file for exercise recognition
...
def collectData(exercise, filenum):
    ...
    if not os.path.exists("rep-data/{}".format(exercise)):
        os.makedirs("rep-data/{}".format(exercise), mode=0o777)
        os.chmod("rep-data/{}".format(exercise), mode=0o777)
        ...
        while(True):    
            ...
            if keyboard.is_pressed('shift'): 
                if first:
                    ...
                    writeToFile(exercise, filenum, data, rep_markers)
                    data = [[],[],[],[],[],[]]
                    rep_markers = []
                    filenum+=1
            ...
            if viewing:
                ...
                if (len(curr_data) == 6):
                    ...
                    if collecting:
                        ...
                        if rep_end: 
                            rep_markers.append(1)
                            rep_end = False
                        else: rep_markers.append(0)
